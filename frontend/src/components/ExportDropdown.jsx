...
import { ArrowDownTrayIcon, PrinterIcon, DocumentTextIcon, TableCellsIcon, EyeIcon, XMarkIcon } from '@heroicons/react/24/outline';

const formatDate = () => new Date().toLocaleDateString('id-ID', {
  weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
});

const PRINT_HEAD = (title) => `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${title}</title>
<style>
...
const PRINT_FOOT = `\n<div class="print-footer">Dokumen ini dicetak dari Sistem Informasi KUD Desa Sari Subur<br>&copy; ${new Date().getFullYear()} &mdash; KUD Desa Sari Subur</div></body></html>`;

function buildPrintHtml(title, content) {
  return `${PRINT_HEAD(title)}${content || '<div class="empty-state">Tidak ada data</div>'}${PRINT_FOOT}`;
}
...
export default function ExportDropdown({ title, fetchAll, filename, pdfUrl, csvUrl, renderPrintContent }) {
  ...
  const getContent = async () => {
    if (typeof renderPrintContent !== 'function') return '';
    const allData = await fetchAll();
    return renderPrintContent(allData);
  };
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      win.document.write(html + '<script>window.onload=function(){setTimeout(function(){window.print()},500)};window.onafterprint=function(){setTimeout(function(){window.close()},300)};<\/script>');
      win.document.close();
    } catch (err) {
  ...
  const handlePreview = async () => {
    ...
    try {
      const content = await getContent();
      setPreviewHtml(buildPrintHtml(title, content));
      setPreviewOpen(true);
    } catch (err) {
  ...
  return (
    <>
      ...
      {previewOpen && (
        <div className="fixed inset-0 z-[99999] bg-black/60 flex items-center justify-center p-4" onClick={closePreview}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { setOpen(false); handlePrint(); }}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-500 text-white text-sm font-semibold hover:from-emerald-700 hover:to-emerald-600 transition-all cursor-pointer">
                  <PrinterIcon className="w-4 h-4" /> Cetak
                </button>
                {pdfUrl && (
                  <button onClick={() => handleDownload(pdfUrl, 'pdf')}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition-all cursor-pointer">
                    <DocumentTextIcon className="w-4 h-4" /> PDF
                  </button>
                )}
                {csvUrl && (
                  <button onClick={() => handleDownload(csvUrl, 'csv')}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-green-600 text-white text-sm font-semibold hover:bg-green-700 transition-all cursor-pointer">
                    <TableCellsIcon className="w-4 h-4" /> CSV
                  </button>
                )}
                <button onClick={closePreview}
                  className="p-2 rounded-xl hover:bg-gray-100 transition-all cursor-pointer" title="Tutup">
                  <XMarkIcon className="w-5 h-5 text-gray-500" />
                </button>
              </div>
            </div>
            <div className="flex-1 bg-gray-50 p-4 overflow-auto">
              <iframe ref={iframeRef} srcDoc={previewHtml} className="w-full h-full bg-white rounded-lg shadow-inner border-0" title="Preview" />
            </div>
          </div>
        </div>
      )}
