    ...
    >
      <td className="py-3 px-3">
        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${
          ...
          {d.tindakan === 'terima' ? <CheckCircle className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
  ...
  return (
    <motion.div variants={containerAnim} initial="hidden" animate="show">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total', value: stats.total, icon: ShieldCheck, color: 'bg-blue-500', shadow: 'shadow-blue-500/20' },
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-border">
            ...
            {filterTindakan && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-primary/10 text-primary rounded-full text-xs font-medium">
                ...
                <button onClick={() => { setFilterTindakan(''); setPage(1); }} className="cursor-pointer hover:text-primary-dark"><X className="w-3 h-3" /></button>
              </span>
            )}
          ...
          <div className="space-y-5">
            ...
            {getTipe(detailModal) === 'Program' && detailModal.verifiable && (
              <div className="pt-2 border-t border-border">
                ...
                >
                  {programExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
