...
import {
  ...
  PlayIcon, PhotoIcon, VideoCameraIcon, EyeIcon,
...
export default function BlogPage() {
  ...
  const handleExpand = useCallback((post) => {
    startTransition(() => {
    ...
    if (post.category) {
      api.blog.related(post.slug).then((res) => {
        if (Array.isArray(res)) setRelatedPosts(res);
      }).catch(() => setError('Gagal memuat artikel terkait'));
    }
    setTimeout(() => {
                    ...
                    <div className="p-6 md:p-8">
                      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-gray-400 flex items-center gap-1">
                            ...
                            {formatViews(viewsCount ?? post.views ?? 0)}
                          </span>
                          {post.author && (
                            <>
                              <span className="text-xs text-gray-300">·</span>
                              <span className="text-xs text-gray-400">oleh {post.author.name || post.author.email}</span>
                            </>
                          )}
                        </div>
                        <button
                          onClick={() => { setExpandedId(null); window.scrollTo({ top: document.getElementById(`post-${post.id}`)?.offsetTop - 120, behavior: 'smooth' }); }}
                          className="p-2 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-foreground transition-colors cursor-pointer"
                        >
                          <XMarkIcon className="w-5 h-5" />
                        </button>
                      </div>

                      <h2 className="font-heading font-bold text-foreground text-xl md:text-2xl mb-4">
                        {post.title}
                      </h2>
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-400">
                            ...
                            {post.media?.filter((m) => m.type === 'video').length > 0 && (
                              <>
                                ...
                                {post.media.filter((m) => m.type === 'video').length} video
                              </>
                            )}
                          </div>
                      ...
                      <div className="mt-6 pt-4 border-t border-border flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="text-xs font-semibold text-primary hover:text-primary-dark transition-colors flex items-center gap-1 cursor-pointer"
                        >
                          Tutup <ChevronUpIcon className="w-3 h-3" />
                        </button>
                      </div>
                    </div>
                  )}
