    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
          className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm bg-white focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 outline-none transition-all hover:border-gray-300">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map((d) => (
            <div key={d.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md hover:border-emerald-100">
              <div className="p-5">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 space-y-4 animate-fade-in">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                          {Object.entries(d.data.dokumen).map(([key, url]) => (
                            <button key={key} onClick={() => { setPreviewImage(url); setPreviewLabel(PERSYARATAN_LABEL[key] || key); }}
                              className="group relative aspect-[3/4] rounded-xl overflow-hidden border border-gray-200 bg-gray-50 cursor-pointer">
                    ...
                    {d.tanda_tangan_digital && (
                      <div>
                        ...
                        <div className="bg-white rounded-lg border border-gray-100 p-4 inline-block">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
