...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-verifikator { background: #dcfce7; color: #166534; }
    .badge-pekebun { background: #fef9c3; color: #854d0e; }
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $i + 1 }}</td>
            <td><strong>{{ $u->name ?? $u->nama ?? '-' }}</strong></td>
            <td>{{ $u->email ?? '-' }}</td>
            <td><span class="badge badge-{{ $u->role }}">{{ $u->role }}</span></td>
            <td>{{ $u->pekebun->status ?? '-' }}</td>
            <td>{{ $u->pekebun->nik ?? '-' }}</td>
            <td>{{ $u->pekebun->no_whatsapp ?? '-' }}</td>
