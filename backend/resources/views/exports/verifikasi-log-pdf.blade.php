...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-terima { background: #dcfce7; color: #166534; }
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $v->verifikator->name ?? 'Sistem' }}</td>
            <td><span class="badge badge-{{ $v->tindakan }}">{{ $v->tindakan }}</span></td>
            <td>{{ class_basename($v->verifiable_type) }} #{{ $v->verifiable_id }}</td>
            <td>{{ $v->catatan ?? '-' }}</td>
        </tr>
        @endforeach
    </tbody>
